...
if($_REQUEST['type']=="Save"){
		...
		{	
			...
			{
				$game_insert = "insert into $_SESSION[user_tem] (USER_ID,SCORE,FLAKES,LEVEL_ID,GAME_DATE) values('$user_temp','$score','$flakes','$level',NOW())";
				$game_insert_result = mysql_query($game_insert) or die("Data not found.");				
			}
			$val = "register";
			$val = trim($val);
		...
		{	
			if(!isset($_SESSION[user_tem])) {
				...
				echo $_SESSION[user_tem]="user_temp".$token;
			...
			echo $level_query = "select * from $_SESSION[user_tem] where LEVEL_ID='$level' and USER_ID='$user_temp'";			
			$level_result = mysql_query($level_query) or die("Data not found.");
			$level_num=mysql_num_rows($level_result);
			...
			if($level_num > 0)
			{
				...
				$score_final=$score1+$score;
				echo $game_update = "update $_SESSION[user_tem] set SCORE='$score_final', FLAKES='$flakes',GAME_DATE=NOW() WHERE USER_ID='$user_temp' and LEVEL_ID='$level'";
				$game_result = mysql_query($game_update) or die("Data not found.");
			}
			else
			{
				echo $game_insert = "insert into $_SESSION[user_tem] (USER_ID,SCORE,FLAKES,LEVEL_ID,GAME_DATE) values('$user_temp','$score','$flakes','$level',NOW())";
				$game_insert_result = mysql_query($game_insert) or die("Data not found.");				
			}			
		}
		echo "insert";
...
//		$xml_output .= "\t\t<music_path>" . "http://nieveenlima.pe/juego_dev/admin/music_file/84efemusic_duplicate.mp3" . "</music_path>\n"; 	
...
//		$xml_output .= "\t\t<noncambg_path>" . "http://nieveenlima.pe/juego_dev/admin/images/noncambg/572b9animated_snow_flakes-300x225.gif" . "</noncambg_path>\n"; 	
...
//		$url_rank = str_replace("data.php","ranking.php",$url_find);
		...
		$xml_output .= "\t\t<url_fb_register>" . $url_notregister . "</url_fb_register>\n"; 
		$url_nonfb = str_replace("data.php","final4.php",$url_find);
		$xml_output .= "\t\t<url_nonfb>" . $url_nonfb . "</url_nonfb>\n";
		
		$url_exit = str_replace("data.php","logout.php",$url_find);
		$xml_output .= "<url>\n";
