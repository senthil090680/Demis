...
/*$level_query = "select * from game where game_id in (select max(game_id) where USER_ID='$user_id')";			
...
//							(SELECT MIN(level_id*1) FROM level WHERE points_beat*1>".$_SESSION['score_flash'].")";
			$rank_query  = "select NICKNAME from level where LEVEL_ID=".$curlevel;
			$rank_result = mysql_query($rank_query) or die("Data not found.");
			$rank_row=mysql_fetch_array($rank_result);
...
<span id="digi-score">
<?PHP 
//$querytot = "select sum(FLAKES) as totalflakes from game USER_ID != 'temp'";
