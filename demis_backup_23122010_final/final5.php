...
$description ='Capturé '.$_SESSION['FLAKES_NEW'].' copos, tú cuántos harías? Ayúdanos a capturar 1 millón para tener nieve en Lima.';
			...
			if($_SESSION[PIC_IMAGE] != '') {
				$pic_query = "select PIC_PATH from user where FACEBOOK_ID='". $facebook_id."'";			
				$pic_result = mysql_query($pic_query) or die("Data not found.");
				$pic_row=mysql_fetch_array($pic_result);
				$pic=$pic_row[PIC_PATH];
				
				$pic_image = "http://nieveenlima.pe/juego_dev/images/user/webcam_picture/$pic";
			}
  $attachment =  array(
...
<div id="post-login">
  ...
  <div class="tus-result">
     ...
     <div class="avatar">
    <?PHP 
$queryimg = "select USER_IMAGE from user where FACEBOOK_ID='{$_SESSION['FACEBOOK_ID']}'";
...
<?php 
$tamaño = getimagesize($resimg['USER_IMAGE']);
$fp = fopen($resimg['USER_IMAGE'], "rb");
if ($tamaño && $fp) { 
?>
	<img src="<?php echo $resimg['USER_IMAGE']; ?>" alt="" style="width:120px;height:80px;" > 
<? }else{ ?>
...
?> copos</span>
      ...
      <span><?PHP //echo $_SESSION['LEVEL_NEW']; 
//			$rank_query  = "select NICKNAME from level where LEVEL_ID=".$_SESSION[LEVEL_NEW]."+1";
			$levels=array(30,75,135,210,300,405,525,650,800,965);
			$i=0;
			$curlevel=0;
			while($i<10 and $curlevel==0)
			{
				if($_SESSION['SCORE_NEW']*1<$levels[$i]*1){
					$curlevel=$i+1;
				}
				$i++;
			}
...
//			$rank_query  = "SELECT NICKNAME FROM level WHERE level_id IN
//							(SELECT MIN(level_id*1) FROM level WHERE points_beat*1>".$_SESSION['score_flash'].")";
 ...
 <?php include "ranking-friends.php"; ?>  
  </div>
