...
if ($me) {
	...
	$sqlinsert=@mysql_query("update user set LAST_LOGIN_DATE_TIME=NOW() where FACEBOOK_ID='{$me['id']}'");
						...
						echo $temp_to_userid_query = "update $_SESSION[user_tem] set USER_ID='$userid' where USER_ID='{$_SESSION[USER_TEMP]}'";
						$temp_to_userid_result = mysql_query($temp_to_userid_query) or die("Data not found.");
						
						echo $userto_game_query = "insert into game (USER_ID,SCORE,FLAKES,LEVEL_ID,GAME_DATE,PIC_PATH) SELECT USER_ID,SCORE,FLAKES,LEVEL_ID,GAME_DATE,PIC_PATH from $_SESSION[user_tem]";
						$userto_game_result = mysql_query($userto_game_query) or die("Data not found.");
						
						echo $drop_query = "drop table $_SESSION[user_tem]";
						$drop_result = mysql_query($drop_query) or die("Data not found.");
						
						echo $game_datas = "select SUM(SCORE) AS game_score, SUM(FLAKES) AS game_flakes from game WHERE USER_ID='$userid'";
						...
						echo $user_insert = "update user set SCORE='$game_score', FLAKES='$game_flakes', LEVEL_ID='{$_SESSION['LEVEL_NEW']}',LASTPLAYED_DATE=NOW() where USER_ID='$userid'";
